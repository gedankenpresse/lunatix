...
pub struct CapCounted<'alloc, 'mem, A: Allocator<'mem>, T: ?Sized>(ManuallyDrop<Box<'alloc, 'mem, A, T>>);
...
impl<'alloc, 'mem, A: Allocator<'mem>, T: ?Sized> Deref for CapCounted<'alloc, 'mem, A, T> {
    type Target = T;
