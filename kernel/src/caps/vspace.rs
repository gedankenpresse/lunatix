...
impl VSpace {
    pub(crate) fn init(mem: &mut caps::Memory) -> Result<caps::Cap<Self>, NoMem> {
        ...
        let root = virtmem::PageTable::init_copy(
            root.cast::<MaybeUninit<mem::Page>>(), 
        unsafe { crate::KERNEL_ROOT_PT.as_ref().expect("No Kernel Root Page Table found") },
        );
    ...
    pub(crate) fn map_range(&self, mem: &mut caps::Memory, vaddr_base: usize, size: usize, flags: usize) -> Result<(), NoMem> {
    ...
    pub(crate) fn map_page(&self, mem: &mut caps::Memory, vaddr: usize, flags: usize) -> Result<(), NoMem> {
